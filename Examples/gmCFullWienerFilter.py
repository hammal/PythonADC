...
# gm_integrator = 1e3 * 1e-8 / 2. # 1/16kOhm
...
model1 = defaultSystems.gmCChain(order, gm1, gm2, C)
model2 = defaultSystems.gmCChain(order, gm1, gm2, C)
...
u = u[::SIM_OSR]
#
# eta12 = np.abs((gm1/C /(1j * 2 * np.pi * fp))**order) ** 2
eta12 = 10 ** (0 * 40/10)
# eta22 = [np.abs(gm1/C/(1j * 2 * np.pi * fp))]
# eta22 = [eta12]
# for i in range(order - 1):
#     # eta22.append(eta22[-1] * eta22[0])
#     eta22.append(eta12)
# eta22 = np.diag(eta22)
